...
sub load()
    if not(m.global.load)
        m.global.load = true
        homeTask = CreateObject("roSGNode", "FetchHome")
        homeTask.ObserveField("response", "setResponse")
        homeTask.setField("page", m.page)
        homeTask.control = "run"
    end if
end sub